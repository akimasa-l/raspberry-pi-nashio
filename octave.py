...
class DisplaySound:
    ...
    def get_index(number):
        return int(round((number-2048)/256))

    @staticmethod
    def get_distance(number):
        return int(round(number*100/4096))
    ...
    def main(self, octave=OCTAVE):
        ...
        ac=ad_converter.Ad_Converter()
        try:
            while 1:
                d1,d2,d3=ac.read_all()
                if d1>d2:
                    self.tm.show("FUCK")
                else:
                    self.args.rangel,self.args.ranger,self.args.distancemax=DisplaySound.get_index(d1),DisplaySound.get_index(d1),DisplaySound.get_distance(d3)
